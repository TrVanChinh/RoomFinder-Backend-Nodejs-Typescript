...
export default class AuthRoute implements Route {
  ...
  private initializeRoutes() {
    this.router.post(this.path, this.authController.adminLogin); //POST: http://localhost:5000/api/auth
    this.router.post(this.path + '/user', this.authController.userLogin); 
